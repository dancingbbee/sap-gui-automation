...
- **Claude Code** 설치 — 이 플러그인은 Claude Code 확장입니다 (없으면 먼저 설치)
- **SAP GUI** 설치 + 평소처럼 로그인 가능 — Windows = SAP GUI for Windows / macOS = SAP GUI for Java
- **Windows만**: Python 3 (없으면 [python.org](https://www.python.org)에서 설치 — `pywin32`/`Pillow`는 설치 스크립트가 자동으로 깔아줍니다)
- **GitHub(`github.com`) 접근 가능한 네트워크** — 사내망에서 막히면 IT/프록시 확인
